	...
	if (num_domain > 0)
	{
		...
		else
		{
			...
			for (int i = 0; i < num_domain; i++)
			{
				...
				double diff_utilization = fabs(vcpu_mappings[i].utilization - vcpu_mappings[next_index].utilization);
				printf("Inside is_uneven for loop, diff_utilization = %.2f\n", diff_utilization);
				if (diff_utilization > 20)
				{
					is_uneven = 1;
					break;
				}
