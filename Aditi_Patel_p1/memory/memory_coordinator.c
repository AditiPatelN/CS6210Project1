			...
			for (int i = 0; i < num_domain; i++)
			{
				...
				else
				{
					...
					else
					{
						...
						virDomainMemoryStatPtr stats = calloc(VIR_DOMAIN_MEMORY_STAT_NR, sizeof(*stats));
						int mem_stats = virDomainMemoryStats(domain, stats, VIR_DOMAIN_MEMORY_STAT_NR, 0);
						if (mem_stats < 0)
						{
						...
						else
						{
							...
							for (int j = 0; j < mem_stats; j++)
							{
								...
								{
									unused_mem = (stats[j].val) / 1024; // in MB
								}
								else if (stats[j].tag == VIR_DOMAIN_MEMORY_STAT_USABLE)
								{
									balloon_usable = (stats[j].val) / 1024;
								}
							}
							...
							if (unused_mem <= 150 && mem_used != domain_max_mem)
							{ // or 100
							...
							else if ((mem_used == domain_max_mem) || (unused_mem > 200))
							{
